  ...
  try {
    ...
    for (const rider of riders) {
      if (rider.assignedBikeId) {
        ...
        console.log(`Initialized location for rider: ${rider.fullName}`);
      }
    }
    
    console.log('All rider locations initialized successfully');
  } catch (error) {
...
export async function initializeBikeLocationForNewRider(riderId: string) {
  try {
    ...
    if (rider && rider.assignedBikeId) {
      ...
      console.log(`Initialized location for new rider: ${rider.fullName}`);
    }
  } catch (error) {

...
async function clearStorage() {
  ...
  try {
    ...
    const folderPromises = res.prefixes.map(async (folderRef) => {
      ...
      const folderDeletePromises = folderContents.items.map(itemRef => deleteObject(itemRef));
      return Promise.all(folderDeletePromises);
    });
    await Promise.all(folderPromises);
    
    console.log(`✅ Cleared ${res.items.length} files from Storage`);
  } catch (error) {
    console.log('⚠️  Storage is already empty or error occurred:', error.message);
  }
}
...
async function main() {
  try {
    ...
    // Collections to clear (keeping only essential structure)
    const collectionsToKeep = ['users']; // Keep user accounts but clear other data

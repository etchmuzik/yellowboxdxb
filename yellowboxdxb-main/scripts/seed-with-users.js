...
/**
 * Enhanced seed script that creates both Firebase Auth users and Firestore data
 * This resolves the "Failed to load riders" issue completely
 */
...
async function seedDatabase() {
  try {
    ...
    // Create Firebase Auth users and user documents
    for (const user of testUsers) {
      try {
        // Create Firebase Auth user
        const userCredential = await createUserWithEmailAndPassword(auth, user.email, user.password);
        console.log(`✅ Created auth user: ${user.email} (UID: ${userCredential.user.uid})`);
        
        // Create user document in Firestore
        await setDoc(doc(db, 'users', userCredential.user.uid), user.userData);
        console.log(`✅ Created user document: ${user.email} (${user.userData.role})`);
    ...
    console.log(`📈 Created ${testUsers.length} users, ${sampleRiders.length} riders, and ${sampleExpenses.length} expenses`);
    console.log('');

...
struct CoachChatView: View {
                ...
                ScrollView {
                    LazyVStack(spacing: 16) {
                        ForEach(viewModel.messages) { message in
                            MessageRow(message: message) { quickAction in
    ...
    private func sendMessage(_ overrideText: String? = nil) {
        let textToSend = overrideText ?? messageText
        guard !textToSend.isEmpty else { return }
        
        viewModel.sendMessage(textToSend, totalCalories: dataManager.totalCalories)
        if overrideText == nil {
            messageText = ""
        }
    }
    
    private func triggerQuickAction(_ action: String) {
        sendMessage(action)
    }
}
